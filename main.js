...
function delUser() {
    if (event.target.dataset.name == 'delete') {
        dataArray.splice(+event.target.dataset.id, 1);
...
//EDIT BUTTON
function editUser1() {
    if (event.target.dataset.name == 'edt') {
        getID('btn1').style.visibility = "hidden";
        getID('btn2').style.visibility = "visible";
        getID("login").value = dataArray[event.target.dataset.id].login;
        getID("password").value = dataArray[event.target.dataset.id].password;
        getID("email").value = dataArray[event.target.dataset.id].email;
        let userIndex = +event.target.dataset.id;
